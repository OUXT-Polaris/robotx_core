...
void field_map_server::load_()
{
    ...
    std::string obstacles_csv = ros::package::getPath("robotx_navigation") + "/data/obstacles.csv";
    ...
    std::ifstream obstacles_ifs(obstacles_csv.c_str());
    std::string obstacles_line;
    i = 0;
    while (getline(obstacles_ifs, obstacles_line))
    {
        std::vector<std::string> strvec = split_(obstacles_line, ',');
        geometry_msgs::Point p;
        p.x = std::stod(strvec[0]);
        p.y = std::stod(strvec[1]);
        p.z = std::stod(strvec[2]);
        field_map_.obstacles.push_back(p);
        visualization_msgs::Marker marker_msg;
        marker_msg.header.frame_id = map_frame_;
        marker_msg.ns = "obstacles_buoy";
        marker_msg.action = marker_msg.ADD;
        marker_msg.type = marker_msg.MESH_RESOURCE;
        marker_msg.id = i;
        marker_msg.pose.position = p;
        ...
        marker_msg.color.a = 1;
        marker_msg.frame_locked = true;
        marker_msg.mesh_resource = "package://rviz_plugin_tutorials/media/flag.dae";
        marker_msg.mesh_use_embedded_materials = true;
        markers_.markers.push_back(marker_msg);
        i++;
    }
